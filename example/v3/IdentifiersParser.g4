...
rollupStandard: (KW_ROLLUP | KW_CUBE) LPAREN expression (
		COMMA expression
	)* RPAREN;
...
rollupOldSyntax:
	expressionsNotInParenthesis (
		...
		KW_GROUPING KW_SETS LPAREN groupingSetExpression (
...
expressionsNotInParenthesis: expression expressionPart?;

expressionPart: (COMMA expression)+;
...
floorExpression:
	KW_FLOOR LPAREN expression (KW_TO floorDateQualifiers)? RPAREN;
...
extractExpression:
	KW_EXTRACT LPAREN timeQualifiers KW_FROM expression RPAREN;
...
precedenceLogicOperator: KW_AND | KW_OR;

//precedenceFieldExpression
// precedenceUnaryPrefixExpression precedenceUnarySuffixExpression precedenceBitwiseXorExpression
// precedenceStarExpression precedencePlusExpression precedenceConcatenateExpression
// precedenceAmpersandExpression precedenceBitwiseOrExpression precedenceSimilarExpressionMain
// precedenceSimilarExpression precedenceEqualExpression precedenceNotExpression
// precedenceAndExpression precedenceOrExpression
expression:
	atomExpression (
...
functionIdentifier: identifier DOT identifier | identifier;

principalIdentifier: identifier | QuotedIdentifier;

// Here is what you have to do if you would like to add a new keyword. Note that non reserved
// keywords are basically the keywords that can be used as identifiers. (1) Add a new entry to
// HiveLexer, e.g., KW_TRUE : 'TRUE'; (2) If it is reserved, you do NOT need to change
// IdentifiersParser.g because all the KW_* are automatically not only keywords, but also reserved
// keywords. However, you need to add a test to TestSQL11ReservedKeyWordsNegative.java. Otherwise it
// is non-reserved, you need to put them in the nonReserved list below. If you are not sure, please
// refer to the SQL2011 column in
